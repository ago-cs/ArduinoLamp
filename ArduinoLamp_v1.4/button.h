...
static IRrecv irrecv(CONTROL_PIN);
...
static struct remote_t // структура менеджера питания
{
  ...
  bool cmds_repeat(uint64_t cmd, uint8_t cmd_step = 1U) //блок комманд для нажатых кнопок с удерживанием
  {
    ...
    {
    case IR_BRIGHT_DOWN:
     modes[currentMode].Brightness = constrain(modes[currentMode].Brightness + (modes[currentMode].Brightness / 25 + 1) *  - 1, 1 , 255);
    ...
    case IR_SPEED_DOWN:
modes[currentMode].Speed = constrain(modes[currentMode].Speed + (modes[currentMode].Speed / 25 + 1) * - 1, 1 , 255);
    ...
    case IR_SCALE_DOWN:
      modes[currentMode].Scale = constrain(modes[currentMode].Scale + (modes[currentMode].Scale / 25 + 1) * - 1, 1 , 255);
